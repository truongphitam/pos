...
class PLL_Translation_Post_Model {
	...
	protected function create_post_translation( $data_import, $source_post, $target_language ) {
		$tr_post = $this->clone_source_post( $source_post );
		...
		$tr_id = wp_insert_post( wp_slash( $tr_post_args ) );
		...
		$this->model->post->set_language( $tr_id, $target_language );
		$this->save_translations_group( $source_post->ID, $tr_id, $target_language->slug );
	...
	protected function clone_source_post( $source_post ) {
		$tr_post     = clone $source_post;
		$tr_post->ID = 0;

		// Set back fields values to default when sync is off for the post date.
		if ( ! in_array( 'post_date', $this->model->options['sync'], true ) ) {
			unset( $tr_post->post_date );
			unset( $tr_post->post_date_gmt );
			unset( $tr_post->post_modified );
			unset( $tr_post->post_modified_gmt );
		}
